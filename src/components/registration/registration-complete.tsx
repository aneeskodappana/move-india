  ...
  return (
    <main className="grid min-h-screen place-items-center bg-canvas px-5 py-10">
      <Card className="w-full max-w-2xl p-7 text-center sm:p-10">
        ...
        <p className="mx-auto mt-4 max-w-lg text-base leading-7 text-stone-700">Your verified demo identity <span className="font-mono">{phone}</span> is now linked to a property. M3 will put today’s collection broadcast here.</p>
        <div className="mt-8 rounded-control border border-marigold-300 bg-marigold-100 p-4 text-sm font-bold text-forest-950">DEV MODE remains active. Authentication, addresses, occupants, and collection data are synthetic.</div>
      </Card>
    </main>
  );
}
